...
Write-Host "   MEMSHAK CDN INSTALLER v2.1 ENHANCED" -ForegroundColor Green  
Write-Host "==========================================" -ForegroundColor Green
...
if (-NOT ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    Write-Host "• Install PowerShell 7, Docker Desktop, and WSL"
    Write-Host "• Configure system services and startup"
    Write-Host "• Install SSL certificates"
    Write-Host ""
...
if ($wslInstalled) {
    ...
    Write-ErrorMsg "Skipping Docker installation due to WSL2 dependency issues"
}

if (-not $prereqSuccess) {
    Write-ErrorMsg "Some prerequisites could not be installed"
    Write-Host "Please restart your computer and try again, or install manually:"
...
try {
    ...
    Write-ErrorMsg "Failed to download deployment package: $_"
    Read-Host "Press Enter to exit"
    exit 1
}
...
try {
    ...
    Write-ErrorMsg "Failed to extract deployment package: $_"
    Read-Host "Press Enter to exit"
    exit 1
}
...
Write-Host "🐳 Docker Services: Configured for auto-start" -ForegroundColor Green
Write-Host ""
...
Write-Host "   • Navigate to $InstallDir and run: docker-compose up -d" -ForegroundColor Green
Write-Host ""
